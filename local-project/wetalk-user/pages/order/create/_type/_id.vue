...
import {
    ...
    RecordCourseCreateOrder,
    RecordCourseConfirmOrder,
    TYPE_OFFLINE_COURSE_ORDER,
    OfflineCourseConfirmOrder,
    OfflineCourseCreateOrder,
    TYPE_ONLINE_COURSE_ORDER,
    OnlineCourseCreateOrder,
    OnlineCourseConfirmOrder,
    OrderPay,
...
const courseType = computed(() => {
    return route.value.params.type;
});
...
function toPay() {
    ...
    switch (courseType.value) {
        ...
        case "offline-course":
            OfflineCourseCreateOrder({
                offlineCourseid: courseId.value,
            })
                .then((res) => {
                    if (res.statusCode !== 0) return;
                    orderId.value = res.data.orderid;
            ...
            break;
        case "online-live-course":
            OnlineCourseCreateOrder({
                onlineCourseid: courseId.value,
            })
                .then((res) => {
                    if (res.statusCode !== 0) return;
                    orderId.value = res.data.orderid;
